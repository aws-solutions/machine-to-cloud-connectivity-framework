...
class ModbusDataCollectionController:
    ...
    def _read_coils(self, message_batches: dict, address: int, count: int, slave_address: int, modbus_host_tag: str) -> dict:
        ...
        if read_coils_response is not None:
            ...
            tag = f'{modbus_host_tag}_readCoils_{slave_address}'
            message_batches = self._add_message_to_batches(
                tag, message_batches, message)
        else:
            self.logger.warning(
                f'Did not get a response for reading coils for slave: {slave_address}')
        return message_batches

    def _read_discrete_inputs(self, message_batches: dict, address: int, count: int, slave_address: int, modbus_host_tag: str) -> dict:
        ...
        if read_descrete_inputs_response is not None:
            ...
            tag = f'{modbus_host_tag}_readDiscreteInputs_{slave_address}'
            message_batches = self._add_message_to_batches(
                tag, message_batches, message)
        else:
            self.logger.warning(
                f'Did not get a response for reading discrete inputs for slave: {slave_address}')
        return message_batches

    def _read_holding_registers(self, message_batches: dict, address: int, count: int, slave_address: int, modbus_host_tag: str) -> dict:
        ...
        if read_holding_registers_response is not None:
            ...
            tag = f'{modbus_host_tag}_readHoldingRegisters_{slave_address}'
            message_batches = self._add_message_to_batches(
                tag, message_batches, message)
        else:
            self.logger.warning(
                f'Did not get a response for reading holding registers for slave: {slave_address}')
        return message_batches

    def _read_input_registers(self, message_batches: dict, address: int, count: int, slave_address: int, modbus_host_tag: str) -> dict:
        ...
        if read_input_registers_response is not None:
            ...
            tag = f'{modbus_host_tag}_readInputRegisters_{slave_address}'
            message_batches = self._add_message_to_batches(
                tag, message_batches, message)
        else:
            self.logger.warning(
                f'Did not get a response for reading input registers for slave: {slave_address}')
        return message_batches

    def _get_modbus_data(self, modbus_slave_config: ModbusSlaveConfig, message_batches={}) -> dict:
        if modbus_slave_config.do_read_coils:
            message_batches = self._read_coils(message_batches, modbus_slave_config.read_coils_address, modbus_slave_config.read_coils_count,
                                               modbus_slave_config.slave_address, modbus_slave_config.modbus_host_tag)

        if modbus_slave_config.do_read_discrete_inputs:
            message_batches = self._read_discrete_inputs(message_batches, modbus_slave_config.read_discrete_inputs_address,
                                                         modbus_slave_config.read_discrete_inputs_count, modbus_slave_config.slave_address, modbus_slave_config.modbus_host_tag)

        if modbus_slave_config.do_read_holding_registers:
            message_batches = self._read_holding_registers(message_batches, modbus_slave_config.read_holding_registers_address,
                                                           modbus_slave_config.read_holding_registers_count, modbus_slave_config.slave_address, modbus_slave_config.modbus_host_tag)

        if modbus_slave_config.do_read_input_registers:
            message_batches = self._read_input_registers(message_batches, modbus_slave_config.read_input_registers_address,
                                                         modbus_slave_config.read_input_registers_count, modbus_slave_config.slave_address, modbus_slave_config.modbus_host_tag)
        return message_batches
    ...
    def _execute_data_retrieval(self, modbus_slave_config: ModbusSlaveConfig, message_batches: dict = {}) -> dict:
        modbus_message_batches = self._get_modbus_data(
            modbus_slave_config, message_batches)
    ...
    def data_collection_control(self, modbus_slave_config: ModbusSlaveConfig, message_batches: dict = {}, iteration: int = 0, error_count: int = 0) -> None:
        """
        ...
        if config.control == 'stop':
            ...
            try:
                self._init_client(modbus_slave_config.modbus_host_url,
                                  modbus_slave_config.modbus_host_port)
            ...
            Timer(
                interval=modbus_slave_config.frequency_in_seconds,
                function=self.data_collection_control,
                args=[modbus_slave_config, message_batches,
                      current_iteration, current_error_count]
            ).start()

    def _handle_get_data_error(self, modbus_slave_config: dict, error: Exception, error_count: int) -> int:
        """
        ...
        """
        self.logger.error(f"Unable to read from modbus slave: {error}")
