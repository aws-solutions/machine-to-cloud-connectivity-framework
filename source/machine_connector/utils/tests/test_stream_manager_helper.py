...
def test_read_from_stream(mocker):
    ...
    mock_read = mocker.patch.object(
        sm_client, "read_from_stream", return_value=Message())
...
def test_read_from_stream_notenoughmessages():
    stream_name = "test_gg_stream"
    sequence = 12345
    read_msg_number = 1
    expected_response = []
    from stream_manager_helper import StreamManagerHelperClient
    sm_client = StreamManagerHelperClient()
    with mock.patch.object(sm_client, "read_from_stream", side_effect=NotEnoughMessagesException):
        with pytest.raises(Exception) as excinfo:
            response = sm_client.read_from_stream(
                stream_name=stream_name, sequence=sequence, read_msg_number=read_msg_number)
...
def test_get_oldest_sequence_number():
    stream_name = "test_gg_stream"
    from stream_manager_helper import StreamManagerHelperClient
    test_sm_client = StreamManagerHelperClient()
...
def test_get_oldest_sequence_number_no_stream():
    from stream_manager_helper import StreamManagerHelperClient
    test_sm_client = StreamManagerHelperClient()
    with pytest.raises(Exception):
...
def test_get_last_sequence_number():
    stream_name = "test_gg_stream"
    from stream_manager_helper import StreamManagerHelperClient
    test_sm_client = StreamManagerHelperClient()
...
def test_get_last_sequence_number_no_stream():
    from stream_manager_helper import StreamManagerHelperClient
    test_sm_client = StreamManagerHelperClient()
    with pytest.raises(Exception):
