...
export default function MessageModal(props: MessageModalProps): JSX.Element {
   ...
   */
  useEffect(() => {
    if (modalType === MessageModalType.CONFIRM && !confirmAction) {
      ...
      <Modal.Footer>
        {
          ...
          <Button id="message-modal-close-button" onClick={() => handleConfirm()} className="uppercase-text" size="sm">{I18n.get('ok')}</Button>
        }
        {
          modalType === MessageModalType.CONFIRM &&
          <>
            <Button id="message-modal-confirm-button" onClick={() => handleConfirm()} className="uppercase-text" size="sm" variant="primary">{I18n.get('confirm')}</Button>
            <Button id="message-modal-cancel-button" onClick={() => hide()} className="uppercase-text" size="sm" variant="secondary">{I18n.get('cancel')}</Button>
          </>
        }
      </Modal.Footer>
    </Modal>
